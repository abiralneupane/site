<?php
	...
	function agileholic_add_icon_box() {
		...
		add_meta_box( 'agileholic-tesimonials-box', __( 'Person Details', 'agileholic' ), 'agileholic_tesimonials_box_callback', 'ag-testimonials', 'side' );
		add_meta_box( 'agileholic-language-box', __( 'Percentage', 'agileholic' ), 'agileholic_language_box_callback', 'ag-language' );
	}
	...
	function agileholic_tesimonials_box_callback( $post ){
		wp_nonce_field( 'agileholic_testimonial_box_data', 'agileholic_testimonial_box_nonce' );
		...
		echo '<table>';
			echo '<tr>';
				echo '<th><label for="position">'.__( 'Position', 'agileholic' ).'</label>:</th>';
				echo '<td><input type="text" id="position" name="position" value="' .$position. '" /></td>';
			echo '</tr>';
			echo '<tr>';
				echo '<th><label for="company">'.__( 'Company', 'agileholic' ).'</label>:</th>';
				echo '<td><input type="text" id="company" name="company" value="' .$company. '" /></td>';
			echo '</tr>';
		echo '</table>';

	}

	function agileholic_language_box_callback( $post ){
		wp_nonce_field( 'agileholic_language_box_data', 'agileholic_language_box_nonce' );

		$language_level = get_post_meta( $post->ID, 'language_level', true );
		
		echo '<table>';
			echo '<tr>';
				echo '<th><label for="language_level">'.__( 'Language level', 'agileholic' ).'</label>:</th>';
				echo '<td><input type="text" id="language_level" name="language_level" value="' .$language_level. '" /></td>';
			echo '</tr>';
		echo '</table>';
	}
	...
	function agileholic_save_icon_box_data( $post_id ) {
		...
		}else if( wp_verify_nonce( $_POST['agileholic_testimonial_box_nonce'], 'agileholic_testimonial_box_data' ) ){
		...
		}else if( wp_verify_nonce( $_POST['agileholic_language_box_nonce'], 'agileholic_language_box_data' ) ){
			
			$language_level = isset( $_POST['language_level'] )? $_POST['language_level']: '';
			
			update_post_meta( $post_id, 'language_level', $language_level );
		}
	}
